...
# keep HD and HCV
hd_hcv <- d
hd_hcv$gene_name <- hd_hcv$gene_key
hd_hcv$gene_key <- NULL
colnames(hd_hcv) <- c("condition", "cell", "gene_usage_count", "sample_id", "gene_name")

hd_hcv <- aggregate(gene_usage_count~sample_id+condition+gene_name+cell, 
                    data = hd_hcv, FUN = sum)


# data_ighv <- list(hd = hd, hcv = hcv, hd_hcv = hd_hcv)
# save(data_ighv, file = "dgu_Tucci/data_ighv.RData")
data_ighv <- get(load("dgu_Tucci/data_ighv.RData"))


dgu_hd <- IgGeneUsage::DGU(ud = data_ighv$hd,
                           mcmc_warmup = 1500,
                           mcmc_steps = 3000,
                           mcmc_chains = 5,
                           mcmc_cores = 5,
                           adapt_delta = 0.95,
                           max_treedepth = 13)
save(dgu_hd, file = "dgu_Tucci/DGU_hd_ighv_rep1.RData")
