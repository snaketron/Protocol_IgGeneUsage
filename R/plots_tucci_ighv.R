...
dgu_hd <- get(load("dgu_Tucci/DGU_hd_ighv_rep1.RData"))
...
ggsave(filename = "results_Tucci_ighv/gu_hd.pdf", 
       plot = plot_gu_hd,
       device = "pdf",
       width = 7.5,
       height = 4)
...
pdf(file = "results_Tucci_ighv/tree_hd.pdf",
    width = 7, height = 4.9)
...
gn <- ggplot(data = x_hd[x_hd$gene_name %in% c("3-72", "6-1", "3-7", "3-74"),])+
...
gp <- ggplot(data = x_hd[x_hd$gene_name %in% c("1-69", "1-24", "1-58", "4-34", "1-45"),])+
  facet_wrap(facets = ~gene_name, scales = "free_x", nrow = 1)+
  geom_hline(yintercept = 0)+
  geom_point(aes(x = '', y = mult*es_mean, col = contrast),
             position = position_dodge(width = 1), size = 1)+
...
g <- (gp|gn)+patchwork::plot_layout(widths = c(5, 4))

ggsave(filename = "results_Tucci_ighv/tree_clusters.pdf", 
       plot = g,
       device = "pdf",
       width = 6,
       height = 2.35)
...
ggsave(filename = "results_Tucci_ighv/tree_example.pdf", 
       plot = heatmap,
       device = "pdf",
       width = 6,
       height = 2.6)
